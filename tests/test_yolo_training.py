...
class _Model:
    """Enough of an ultralytics model to check which callbacks survive."""

    def __init__(self) -> None:
        def mlflow_callback() -> None: ...

        def tensorboard_callback() -> None: ...

        mlflow_callback.__module__ = MLFLOW_CALLBACKS
        tensorboard_callback.__module__ = "ultralytics.utils.callbacks.tensorboard"
        self.callbacks = {"on_train_end": [mlflow_callback, tensorboard_callback]}


def test_silencing_tracking_drops_only_the_mlflow_callbacks() -> None:
    model = _Model()

    silence_tracking(model)

    survivors = [fn.__module__ for fn in model.callbacks["on_train_end"]]
    assert survivors == ["ultralytics.utils.callbacks.tensorboard"]
